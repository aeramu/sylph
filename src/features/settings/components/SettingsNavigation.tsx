...
export type SettingsSection = 'projects' | 'provider' | 'git' | 'permissions' | 'notifications' | 'skills' | 'extensions';
...
function Icon(props: { kind: SettingsSection }) {
  return <span class={`settings-menu-icon ${props.kind}`} aria-hidden="true"><svg viewBox="0 0 24 24" fill="none">
    ...
    <Show when={props.kind === 'provider' || props.kind === 'permissions'}><path d="M12 3.75 5.75 6.5v5.25c0 4.15 2.62 7.22 6.25 8.5 3.63-1.28 6.25-4.35 6.25-8.5V6.5L12 3.75Z"/><path d="M9.25 12.25 11.1 14l3.65-4"/></Show>
...
export default function SettingsNavigation(props: { active: SettingsSection; onSelect: (section: SettingsSection) => void; onClose: () => void }) {
  const sections: Array<[SettingsSection, string]> = [['projects', 'Projects'], ['provider', 'Provider'], ['git', 'Git'], ['permissions', 'Permissions'], ['notifications', 'Notifications'], ['skills', 'Skills'], ['extensions', 'Extensions']];
