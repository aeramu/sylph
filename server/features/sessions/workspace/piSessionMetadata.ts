...
export function appendWorkspaceMetadata(sessionManager: SessionHistoryHandle, binding: SessionBinding) {
...
export function getWorkspaceMetadata(sessionManager: SessionHistoryHandle): SylphWorkspaceMetadata | undefined {
 ...
 */
export function reconcileSessionBinding(sessionManager: SessionHistoryHandle, sessionFile = sessionManager.getSessionFile?.()): SessionBinding | undefined {
...
async function recoverBindingIndex() {
  const sessions = await piSessionHistory.listAll();
  for (const session of sessions) {
    if (!fs.existsSync(session.path)) continue;
    try {
      const manager = piSessionHistory.open(session.path);
