...
describe("multi-directory projects", () => {
  ...
  it("creates unique aliases without storing a default directory", () => {
    const project = projects.createProject({
      ...
      directories: [
        ...
        { name: "app", path: "/tmp/backend" },
      ],
    });
    expect(project.name).toBe("Product");
    expect(project.directories.map((directory) => directory.name)).toEqual(["app", "app-2"]);
    expect(project.path).toBe(path.resolve("/tmp/frontend"));
    expect(project.activeDirectoryId).toBeUndefined();
  });

  it("ignores a legacy stored primary directory", () => {
    fs.writeFileSync(storeFile, JSON.stringify([{
      id: "legacy-multi",
      name: "Legacy multi-root",
      path: "/tmp/backend",
      primaryDirectoryId: "api",
      directories: [
        { id: "web", name: "web", path: "/tmp/web" },
        { id: "api", name: "api", path: "/tmp/backend" },
      ],
    }]));
    const project = projects.getProjects()[0];
    expect(project.path).toBe(path.resolve("/tmp/web"));
    expect(project).not.toHaveProperty("primaryDirectoryId");
  });
  ...
  it("updates a project while preserving existing directory ids", () => {
    ...
    const updated = projects.updateProject(existing, {
      name: "Product 2",
      directories: [
        { id: existing.directories[1].id, name: "backend", path: "/tmp/api" },
    ...
    expect(updated.activeDirectoryId).toBeUndefined();
  });
  ...
  it("creates an active-directory project view without losing other roots", () => {
    ...
    expect(view.activeDirectoryId).toBe(project.directories[1].id);
